  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {photos.map((photo, index) => (
          <div
            ...
            className={`relative group aspect-square rounded-xl overflow-hidden bg-slate-100 border-2 ${
              index === 0 ? "ring-2 ring-[#671E1F]" : "border-transparent"
            }`}
          >
            <img
              src={photo?.url ?? photo}
            ...
            {index === 0 && (
              <span className="absolute top-1 left-1 text-[10px] font-bold bg-[#671E1F] text-white px-1.5 py-0.5 rounded">
                Cover
              </span>
            )}
          </div>
        ...
        <label
          className="aspect-square rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-[#671E1F] hover:bg-slate-50 transition-colors"
